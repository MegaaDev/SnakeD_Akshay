...
const snakeCreatorv = () => {
  for (let i = 1; i <= 35; i++) {
    abcda = document.createElement(`div`);
    abcda.className = `snake${i}v hideSnake`;
    document.querySelector(".gameBox").append(abcda);
  }
};
snakeCreatorv();
...
// let movingObstacle1X;
// let movingObstacle1Y;

// const movingObstacle1 = () => {
//   for (let i = 0; i < 1; i++) {
//     movingObstacle1X = Math.max(
//       Math.floor(Math.random() * gridX - diagNumber) + 1,
//       diagNumber
//     );
//     movingObstacle1Y = Math.max(
//       Math.floor(Math.random() * gridY - diagNumber) + 1,
//       diagNumber
//     );
//   }
// };

// let movingObstacle2X;
// let movingObstacle2Y;
// const movingObstacle2 = () => {
//   for (let i = 0; i < 1; i++) {
//     movingObstacle2X = Math.max(
//       Math.floor(Math.random() * gridX - horiNumber) + 1,
//       horiNumber
//     );
//     movingObstacle2Y = Math.floor(Math.random() * gridY);
//   }
// };
// movingObstacle2();

// let movingObstacle3X;
// let movingObstacle3Y;
// const movingObstacle3 = () => {
//   for (let i = 0; i < 1; i++) {
//     movingObstacle3Y = Math.max(
//       Math.floor(Math.random() * gridY - vertiNumber) + 1,
//       vertiNumber
//     );
//     movingObstacle3X = Math.floor(Math.random() * gridX);
//   }
// };
// movingObstacle3();
...
document.addEventListener("keydown", (event) => {
  ...
  if (boolforListener) {
    ...
    } else if (event.key == "s" && velocityYv !== -1) {
      startedBoolean = true;
      velocityXv = 0;
      velocityYv = 1;
      if (!startedTimer) {
    ...
    } else if (event.key == "d" && velocityXv !== -1) {
      startedBoolean = true;
      velocityXv = 1;
      velocityYv = 0;
      if (!startedTimer) {
    ...
    } else if (event.key == "a" && velocityXv !== 1) {
      startedBoolean = true;
      velocityXv = -1;
      velocityYv = 0;
      if (!startedTimer) {
...
snakeDrop();
const snakeDropv = () => {
  snakeXv = Math.floor(Math.random() * gridX) + 1;
  snakeYv = Math.floor(Math.random() * gridY) + 1;
};
snakeDropv();
    ...
    if (ReloadGameNumber === 3) {
      setTimeout(() => {
        ...
        document.querySelector(
          ".player1scoreit"
        ).innerHTML = `Player1: <span>${score1}</span>`;
        document.querySelector(
          ".player2scoreit"
        ).innerHTML = `Player2: <span>${score2}</span>`;
        boolNow = true;
...
let arrayFollowv = [[snakeYv, snakeXv]];
...
let initialXv = r.style.setProperty("--initialXv", `30px`);
let initialYv = r.style.setProperty("--initialYv", `250px`);
...
const gameLoad = () => {
  ...
  if (snakeXv === portals[0][1] && snakeYv === portals[0][0]) {
    document
      .querySelector(".headsnakev")
      .parentElement.removeChild(document.querySelector(".headsnakev"));
    headBoolean = false;
    ...
    //     .parentElement.removeChild(document.querySelector(`.snake${i}`));
    // }
    for (let i = 1; i <= arrayFollowv.length; i++) {
      document.querySelector(`.snake${i}v`).style.transition = "none";
    }
    ...
    setTimeout(() => {
      ...
      //   document.querySelector(".gameBox").append(abcda);
      // }
    }, arrayFollowv.length * speed);
  } else if (snakeXv === portals[1][1] && snakeYv === portals[1][0]) {
    document
      .querySelector(".headsnakev")
      .parentElement.removeChild(document.querySelector(".headsnakev"));
    headBoolean = false;
  ...
  for (let i = 0; i < obstacles.length; i++) {
    ...
    if (snakeXv === obstacles[i][1] && snakeYv === obstacles[i][0]) {
      velocityXv = 0;
      velocityYv = 0;
      seconds = 0;
      boolforListener = false;
  ...
  for (let i = 0; i < arrayOfLetters.length; i++) {
    if (arrayOfLetters[i][0] === snakeYv && arrayOfLetters[i][1] === snakeXv) {
      if (
        document.querySelector(`.letter${i}`).textContent === letters[index]
      ) {
        letterSoundEffect.play();
        ...
        arrayFollowv.push([snakeYv, snakeXv]);
        arrayOfLetters[i] = "";
        console.log(arrayOfLetters);
        speed -= 5;
        ...
        arrayFollowv.unshift([snakeYv, snakeXv]);
        arrayFollowv.unshift([snakeYv, snakeXv]);
        targetValueChanger();
        seconds -= 4;
        speed = Math.max(speed, 10);
  ...
  if (started) {
    for (let i = arrayFollowv.length - 1; i > 0; i--) {
      arrayFollowv[i] = arrayFollowv[i - 1];
    }
    arrayFollowv[0] = [snakeYv, snakeXv];
  }
  ...
  for (let i = 0; i <= arrayFollowv.length - 1; i++) {
    if (i === 0) {
      e = r.style.setProperty(
        "--initialXv",
        `${(arrayFollowv[i][1] - 1) * gridMultiplier}px`
      );
      f = r.style.setProperty(
        "--initialYv",
        `${(arrayFollowv[i][0] - 1) * gridMultiplier}px`
      );
    } else if (i !== 0 && startedBooleanv) {
      if (
        document.querySelector(`.snake${i}v`).classList.contains("hideSnake")
      ) {
        document.querySelector(`.snake${i}v`).classList.remove("hideSnake");
      }

      document.querySelector(`.snake${i}v`).style.top = `${
        (arrayFollowv[i][0] - 1) * gridMultiplier
      }px`;
      document.querySelector(`.snake${i}v`).style.left = `${
        (arrayFollowv[i][1] - 1) * gridMultiplier
      }px`;
      boardSelector.innerHTML = htmlSelector;
    }
  }
  ...
  if (
    ...
    } else if (arrayFollow.length >= 2 && arrayFollow.length < 5) {
  ...
  if (
    arrayFollowv[0][1] === speedBoosterX &&
    arrayFollowv[0][0] === speedBoosterY &&
    booldad1
  ) {
    console.log("cool");
    speed -= 10;
  ...
  if (
    arrayFollowv[0][1] === speedBoosterTimeX &&
    arrayFollowv[0][0] === speedBoosterTimeY &&
    booldad2
  ) {
    console.log("cool");
    seconds += 10;
  ...
  if (
    arrayFollowv[0][1] === speedBoosterMegaX &&
    arrayFollowv[0][0] === speedBoosterMegaY &&
    booldad3
  ) {
    bonusSoundEffect.play();
    if (arrayFollowv.length >= 15) {
      for (let i = arrayFollowv.length - 8; i < arrayFollowv.length; i++) {
        document.querySelector(`.snake${i}v`).classList.add("hideSnake");
      }
      deleteBooster = arrayFollowv.splice(arrayFollowv.length - 8, 7);
    } else if (arrayFollowv.length >= 10 && arrayFollow.length < 15) {
      for (let i = arrayFollowv.length - 6; i < arrayFollowv.length; i++) {
        document.querySelector(`.snake${i}v`).classList.add("hideSnake");
      }
      deleteBooster = arrayFollowv.splice(arrayFollowv.length - 6, 5);
    }
    if (arrayFollowv.length >= 8 && arrayFollowv.length < 10) {
      for (let i = arrayFollowv.length - 5; i < arrayFollowv.length; i++) {
        document.querySelector(`.snake${i}v`).classList.add("hideSnake");
      }
      deleteBooster = arrayFollowv.splice(arrayFollowv.length - 5, 4);
    } else if (arrayFollowv.length < 8 && arrayFollowv.length >= 5) {
      for (let i = arrayFollowv.length - 4; i < arrayFollowv.length; i++) {
        document.querySelector(`.snake${i}v`).classList.add("hideSnake");
      }
      deleteBooster = arrayFollowv.splice(arrayFollowv.length - 4, 3);
    } else if (arrayFollowv.length >= 2 && arrayFollowv.length < 5) {
      for (let i = arrayFollowv.length - 2; i < arrayFollowv.length; i++) {
        document.querySelector(`.snake${i}v`).classList.add("hideSnake");
      }
      deleteBooster = arrayFollowv.splice(arrayFollowv.length - 2, 1);
    }
  ...
  for (let i = 3; i < arrayFollowv.length; i++) {
    if (
      i !== 0 &&
      arrayFollowv[0][0] === arrayFollowv[i][0] &&
      arrayFollowv[0][1] === arrayFollowv[i][1]
    ) {
      if (scoreBool) score1 -= 40;
      scoreBool = false;
      velocityXv = 0;
...
const resetAll = () => {
  ...
  for (let i = 1; i <= 35; i++) {
    ...
    document.querySelector(`.snake${i}v`).style.top = "none";
    document.querySelector(`.snake${i}v`).style.left = "none";
  }
  letterKillCounter = 0;
...
const MovingObstacleLoad = () => {
  ...
  //
  for (let i = 0; i < arrayFollowv.length; i++) {
    if (
      (arrayFollowv[i][1] <= obstacle1X + 1 &&
        arrayFollowv[i][1] >= obstacle1X - 1 &&
        arrayFollowv[i][0] === obstacle1Y) ||
      (arrayFollowv[i][1] <= obstacle3X + 1 &&
        arrayFollowv[i][1] >= obstacle3X - 1 &&
        arrayFollowv[i][0] === obstacle3Y)
    ) {
      score1 -= 40;
      velocityXv = 0;
      velocityYv = 0;
      ...
      arrayFollowv[i][0] <= obstacle2Y + 1 &&
      arrayFollowv[i][0] >= obstacle2Y - 1 &&
      arrayFollowv[i][1] === obstacle2X
    ) {
      score1 -= 40;
      velocityXv = 0;
      velocityYv = 0;
...
//     // displaySet = GameSafe.displaySet;
...
//     // clearInterval(timerTarget);
//     // setInterval(gameLoad, speed);
//     // for (let i = 1; i <= 35; i++) {
//     //   abcda = document.createElement(`div`);
//     //   abcda.className = `snake${i} hideSnake`;
//     //   document.querySelector(".gameBox").append(abcda);
//     // }
//     // r.style.setProperty("--initialX", `15px`);
//     // r.style.setProperty("--initialY", `300px`);
//     // if (boolForBooster) {
//     //   htmlSelector += `<div style = "grid-area: ${speedBoosterY}/${speedBoosterX}"><div class="booster"></div></div>`;
//     // }
...
//     //     );
//     //   } else if (i !== 0 && startedBoolean) {
//     //     // htmlSelector += `<div class = "snake${i}"></div>`;
//     //     if (
//     //       document.querySelector(`.snake${i}`).classList.contains("hideSnake")
//     //     ) {
//     //       document.querySelector(`.snake${i}`).classList.remove("hideSnake");
...
//     //     }px`;
//     //     boardSelector.innerHTML = htmlSelector;
//     //     // r.style.setProperty("--finalX", `${(arrayFollow[i][1] - 1) * 15}px`);
//     //     // r.style.setProperty("--finalY", `${(arrayFollow[i][0] - 1) * 15}px`);
//     //   }
//     // }
